...
import { INVOICE_URL } from '../endpoints';

export function getInvoiceList(query: object){
    return api.get<Invoice[]>(`${INVOICE_URL}${ query ? '?'+(new URLSearchParams( [...Object.entries(query).filter( (f, i) => f[1] )] )) : ''  }`);
}
