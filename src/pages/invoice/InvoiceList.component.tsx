...
function InvoiceListComponent() {
  ...
  const invoiceForm: FormikProps<InvoiceSearch> = useFormik<InvoiceSearch>({
    initialValues: {
    ...
    onSubmit: async (data: InvoiceSearch) => {
      try {
        await getList();
      } catch (e: any) {}
    },
  ...
  const getList = useCallback(async () => {
    ...
    try {
      ...
      const search: any = Object.assign({}, invoiceForm.values);
      ...
      const response = await getInvoiceList(search);
  ...
  }, [invoiceForm.values, invoiceForm.isValid]);
  ...
  const isFormFieldValid = (name: keyof InvoiceSearch) => !!(invoiceForm.touched[name] && invoiceForm.errors[name]);

  const getFormErrorMessage = (name: keyof InvoiceSearch) => {
    return isFormFieldValid(name) && <small className="p-error">{invoiceForm.errors[name]}</small>;
  };
  ...
  const updateFooter = function(rows: any){ 
    ...
    setTimeout( () => {
      ...
      let html = (<ColumnGroup>
        <Row>
            <Column footer="" colSpan={9} align="right" style={{ width: '72rem' }}/>
    ...
    <>
          ...
          <div className="flex gap-4">
            <div className="w-3/4">
              <fieldset className="border-2 rounded-md pb-2">
                <legend className="ml-4 text-lg">Invoice (GSTR2)</legend>
                <div className="flex gap-4 justify-center">
                    <div className="field flex items-center gap-2 w-1/6 max-w-1/6">
                      <label htmlFor="invcType" className="">Type</label>
                      <Dropdown inputId='invcType' id="invcType" name='invcType' options={_invoiceTypes} optionLabel="label" optionValue="value" value={invoiceForm.values.invcType} onChange={(e) => { invoiceForm.setFieldValue("invcType", e.value) }} onBlur={invoiceForm.handleBlur} placeholder='Select Invoice Type'
                    ...
                    <div className="field flex items-center gap-2">
                        ...
                        <Calendar name="fromDate" id="fromDate" value={invoiceForm.values.fromDate} onChange={ (e) => { invoiceForm.setFieldValue('fromDate', e.target.value) } } onBlur={invoiceForm.handleBlur} className={classNames({ 'p-invalid': isFormFieldValid('fromDate'), 'p-inputtext-sm': true })} placeholder="From Date"
                          dateFormat="dd/mm/yy" maxDate={invoiceForm.values.toDate} monthNavigator yearNavigator yearRange={yearRange} mask="99/99/9999"
                          showOnFocus={false}
                        />
                    ...
                    <div className="field flex items-center gap-2">
                        ...
                        <Calendar name="toDate" id="toDate" value={invoiceForm.values.toDate} onChange={ (e) => { invoiceForm.setFieldValue('toDate', e.target.value) } } onBlur={invoiceForm.handleBlur} className={classNames({ 'p-invalid': isFormFieldValid('toDate'), 'p-inputtext-sm': true })} placeholder="To Date"
                          dateFormat="dd/mm/yy" minDate={invoiceForm.values.fromDate} monthNavigator yearNavigator yearRange={yearRange} mask="99/99/9999"
                          showOnFocus={false}
                        />
                    ...
                    <div className="field flex items-center gap-2">
                      <Button type='button' label="Search" className="p-button-raised p-button-sm" onClick={invoiceForm.submitForm} disabled={!invoiceForm.isValid} icon="pi pi-search" />
                    </div>
                </div>
              </fieldset>
            </div>
      ...
      <div className="mx-3 card" style={{ maxHeight: "calc(100vh - 160px)" }}>
        ...
        >
          ...
          <Column field="gst" header="GST" sortable
            filter filterPlaceholder="Search by gst" filterHeaderClassName="p-inputtext-sm"
            showFilterMenu={false}
            style={{ width: '12rem' }}
          ></Column>
          <Column
